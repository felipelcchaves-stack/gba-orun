...
-- Add device columns to profiles (may already exist from partial migration)
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS device_id TEXT;
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS device_changed_at TIMESTAMPTZ;

-- Drop and recreate admin_list_profiles with new return columns
